...
3. **Se restar Pendência Manual** (acumulada no Passo 3.4) em qualquer tarefa, **não** marque a TASK como `done` no backlog de origem (root ou `docs/$SCOPE/context/backlog.md`, conforme o prefixo do ID), mesmo com a verificação automática passando. A Spec fica `approved` com pendência em aberto até o humano rodar `/recheck`.
4. Se tudo passou e não houver Pendência Manual, atualize o `Status` da TASK para `done` no backlog de origem (root ou `docs/$SCOPE/context/backlog.md`).
